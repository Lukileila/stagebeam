...
export const Workspace = ({
  ...
  const [stageDimensions, setStageDimensions] = useState({
    width: 100,
    height: 100,
  }); //initial nonsense Values just so it doesn't cause an error / warning
  ...
  useEffect(() => {
    ...
    if (x.width / x.height > projectionAspectRatio) {
      setAspectToggle(true);
    } else setAspectToggle(false);
  ...
  return (
    <div className='fixed left-[30%] w-[70%] h-[70%] bg-black p-1'>
      <div
        ref={workspace}
        className=' bg-black text-gray-300 h-full w-full p-5 /* group/canvas */ z-1 overflow-hidden rounded-sm'
      >
        <h1 className='absolute text-gray-700 p-2 text-md'>
          Beamer - aspect ratio: {parseFloat(projectionAspectRatio).toFixed(2)}
        </h1>
        {/* formerly the "pink box": */}
        <div
          id='canVas'
          ref={stageContainer}
          className={`relative bg-transparent border-2 border-yellow-400 ${
            aspectToggle && 'h-full'
          } z-30 `}
          draggable={false}
          style={{ aspectRatio: projectionAspectRatio }}
        >
          <ObjectCreator
        ...
        <div
          className={
            'absolute left-0 top-0 w-6 h-full bg-gray-800 mix-blend-darken  z-40'
          }
          draggable='false'
        ></div>{' '}
        {/* left masking overlay */}
        <div
          className={
            'absolute left-0 top-0 h-6 w-full bg-gray-800 mix-blend-darken  z-40'
          }
        ></div>{' '}
        {/* top masking overlay */}
        <div
          className={
            'absolute top-0 h-full w-full bg-gray-800 mix-blend-darken  z-40'
          }
          style={{ left: stageDimensions.width + 1.25 * 20 }}
        ></div>{' '}
        {/* right masking overlay */}
        <div
          className={
            'absolute bottom-0 w-full bg-gray-800 mix-blend-darken z-40'
          }
          style={{
            height:
              window.innerHeight * 0.7 - stageDimensions.height - 1.25 * 20,
          }}
        ></div>{' '}
        {/* bottom masking overlay */}
      </div>
    </div>
  );
};
