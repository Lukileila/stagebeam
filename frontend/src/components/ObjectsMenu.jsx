...
export const ObjectsMenu = ({
  ...
  const [beamerOnline, setBeamerOnline] = useState(
    JSON.parse(localStorage.getItem('beamerOnline'))
  );
  ...
  useEffect(() => {
    const onStorageUpdate = () => {
      setBeamerOnline(
        JSON.parse(localStorage.getItem('beamerOnline')) ?? false
      );
    };
    ...
    <>
      <div className='flex flex-col fixed w-[30%] h-full bg-black p-1 pr-0'>
        <div className='flex flex-row justify-between border-2 border-gray-800 bg-gradient-to-t from-gray-950 to-gray-900 rounded p-1 text-gray-300  cursor-pointer mb-1'>
          {user ? (
            <>
              <NavLink
                to='../dashboard'
                className='flex grow hover:translate-y-px justify-between border outline outline-1 outline-black border-gray-800 hover:border-gray-950 bg-gray-900   rounded px-2 py-1 mr-1'
              ...
              <button
                className='flex grow  justify-between border outline outline-1 outline-black border-gray-800  bg-gray-900   rounded px-2 py-1 mr-1'
                style={{ backgroundColor: '#6B7280' }}
              >
              ...
              <button
                onClick={handleSave}
                className='flex grow hover:translate-y-px justify-between border outline outline-1 outline-black border-gray-800 hover:border-gray-950 bg-gray-900   rounded px-2 py-1 '
            ...
            <>
              ...
              <NavLink
                to='../loginpage'
                className='flex grow  justify-between border outline outline-1 outline-black border-gray-800 bg-gray-900   rounded px-2 py-1 '
                style={{ backgroundColor: '#6B7280' }}
              >
        ...
        <div className='flex flex-col justify-between border-2 border-gray-800 bg-gradient-to-t from-gray-950 to-gray-900 rounded p-1 text-gray-300  cursor-pointer'>
          ...
          <button
            onClick={openBeamer}
            className='flex  hover:translate-y-px justify-between border outline outline-1 outline-black border-gray-800 hover:border-gray-950 bg-gray-900   rounded px-2 py-1 '
            style={{ backgroundColor: beamerOnline ? '#6B7280' : '#FACC15' }}
          >
            <p className=' text-black '>
              {beamerOnline
                ? 'beamer window is open  '
                : 'pop out the beamer window  '}
            </p>
            <img
              src={popoutSvg}
              alt='popout'
        ...
        <div className='grow flex flex-col border-2  border-gray-800 bg-gradient-to-t from-gray-950 to-gray-900 rounded p-1  text-gray-300 z-0'>
          <div className='flex flex-row justify-between'>
            ...
            <div>
              <button
                onClick={exportActiveObjects}
                className='mb-1 border outline outline-1 outline-black border-gray-800 hover:border-gray-950 bg-gray-900  hover:bg-gray-900 rounded px-1 mr-1'
              >
                <p className='text-black hover:translate-y-px'>export</p>
              </button>
              <button
                onClick={clearActiveObjects}
                className=' border outline outline-1 outline-black border-gray-800 hover:border-gray-950 bg-gray-900  hover:bg-gray-900 rounded px-1 mr-1'
              >
